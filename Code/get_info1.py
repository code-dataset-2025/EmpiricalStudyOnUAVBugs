...
# 函数：将提取结果写入 Excel 文件
def write_to_excel(file_name, page_number, info1, info2, info3, info4):
    try:
        # 尝试加载现有文件
        ...
        sheet = wb.active
        sheet.append(["Page Number", "Info1", "Info2", "Info3", "Info4"])  # 添加表头

    # 将结果追加到 Excel 文件中
    sheet.append([page_number, info1, info2, info3, info4])
    wb.save(file_name)
...
for i in range(startnum,len(merged_result)):
    ...
    page_number = str(merged_result[i])  # 根据需要替换为实际编号
    excel_file = "extracted_info.xlsx"
    # 获取网页内容
    response = requests.get(url)
    soup = BeautifulSoup(response.text, "html.parser")
    # 1. 找到第一个class为discussion-sidebar-heading text-bold的div标签
    info1 = None
    span_tag = soup.find('span', class_='css-truncate js-issue-assignees')
    if span_tag:
        # 计算其下属几个p标签
        p_tags = span_tag.find_all('p')
        info1= len(p_tags)

    # 2. 找到第六个class为discussion-sidebar-heading text-bold的div标签
    info2 = None
    div_tags = soup.find_all("div", class_="discussion-sidebar-heading text-bold")
    if len(div_tags) >= 6:
        target_div = div_tags[5]
        match = re.search(r"(\d+)\s+participant", target_div.text)
        if match:
            info2 = int(match.group(1))

    # 3. 找到所有class为d-block comment-body markdown-body js-comment-body的td标签
    info3 = len(soup.find_all("td", class_="d-block comment-body markdown-body js-comment-body"))

    # 4. 找到页面中的第一个和最后一个relative-time标签，并计算时间间隔
    info4 = None
    relative_times = soup.find_all("relative-time")
    if len(relative_times) >= 2:
        first_time = relative_times[0]["datetime"]
        last_time = relative_times[-1]["datetime"]
        # 计算时间差
        time_difference = calculate_time_difference(first_time, last_time)
        info4 = time_difference
    # 4. 写入 Excel 文件
    write_to_excel(excel_file, page_number, info1, info2, info3, info4)
