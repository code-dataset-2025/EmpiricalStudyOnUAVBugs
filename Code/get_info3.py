...
def write_to_excel(file_name, row, data):
    ...
    # 将数据写入指定行
    for col, value in enumerate(data, start=9):  # 从第9列开始
...
def get_user_contributions(username):
    ...
    year_end = datetime(now.year + 1, 1, 1) - timedelta(days=1)
    # 构建GitHub API的URL
    url = f'https://api.github.com/users/{username}/contributions'
    # 添加HTTP头部，包含你的GitHub token
    headers = {
        'Authorization': f'token {api_token}',
    }
    # 发送请求
    response = requests.get(url, headers=headers,
                            params={'to': year_end.strftime('%Y-%m-%d'), 'from': year_start.strftime('%Y-%m-%d')})
    # 检查请求是否成功
    if response.status_code == 200:
        # 计算贡献数量
        contributions = sum(response.json())
        print(f'Total contributions this year: {contributions}')
    else:
